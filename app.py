...
# Create text label for each resort in the bar chart
def bar_text_label(row):
	out_str = '{0}</br>{1}'
	return out_str.format(row.MountainName, row.State)

# Create text label for each resort on the map
def map_text_label(row):
...
def fetch_data(resort_id='All'):
	...
	if resort_id!='All':
		...
		# Create text label for each resort on the map
		resort_data['text'] = resort_data.apply(map_text_label, axis=1)
